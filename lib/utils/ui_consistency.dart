...
class LingAfriqAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final effectiveForegroundColor = foregroundColor ?? 
        (transparent || isDark ? Theme.of(context).colorScheme.onSurface : PanAfricanColors.textPrimary);
...
class LingAfriqIconButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return IconButton(
      icon: Icon(
        icon,
        color: color ?? (isDark ? Theme.of(context).colorScheme.onSurface : PanAfricanColors.textPrimary),
        size: size ?? 24.sp,
      ),
      onPressed: () {
        HapticFeedback.lightImpact();
