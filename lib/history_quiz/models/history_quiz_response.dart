...
class HistoryQuizResponse {
  ...
  final List<HistoryQuiz> results;
  HistoryQuizResponse({
    required this.count,
    required this.next,
  ...
  HistoryQuizResponse copyWith({
    int? count,
    int? next,
    int? previous,
    int? totalscore,
    List<HistoryQuiz>? results,
  }) {
    return HistoryQuizResponse(
  ...
  factory HistoryQuizResponse.fromMap(Map<String, dynamic> map, int? totalScrore) {
    return HistoryQuizResponse(
      count: map['count']?.toInt() ?? 0,
      next: map['next']?.toInt() ?? 0,
      previous: map['previous']?.toInt() ?? 0,
      totalscore: totalScrore ?? 0,
      results: List<HistoryQuiz>.from(map['results']?.map((x) => HistoryQuiz.fromMap(x))),
    );
  }

  String toJson() => json.encode(toMap());

  factory HistoryQuizResponse.fromJson(String source) =>
      HistoryQuizResponse.fromMap(json.decode(source), 0);
  ...
  bool operator ==(Object other) {
    ...
    return other is HistoryQuizResponse &&
        other.count == count &&
        other.next == next &&
...
class HistoryQuiz {
  ...
  factory HistoryQuiz.fromMap(Map<String, dynamic> map) {
    return HistoryQuiz(
      id: map['id']?.toInt() ?? 0,
  ...
  factory HistoryQuiz.fromJson(String source) => HistoryQuiz.fromMap(json.decode(source));
